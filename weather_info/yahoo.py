...
try:
    ...
    for t in today_temp_high_elem:
        print(t.text.strip())
        today_high_temp = re.sub("℃\[.*\]","",t.text.strip())
    ...
    for t in today_temp_low_elem:
        print(t.text.strip())
        today_low_temp = re.sub("℃\[.*\]","",t.text.strip())
    ...
    #明日の最高気温の取得
    tomorrow_temp_high_elem = soup.select("div.forecastCity > table > tr > td:nth-child(2) > div > ul.temp > li.high")

    for t in tomorrow_temp_high_elem:
        print(t.text.strip())
        tomorrow_high_temp  = re.sub("℃\[.*\]","",t.text.strip())

    #明日の最低気温の取得
    tomorrow_temp_low_elem  = soup.select("div.forecastCity > table > tr > td:nth-child(2) > div > ul.temp > li.low")

    for t in tomorrow_temp_low_elem:
        print(t.text.strip())
        tomorrow_low_temp   = re.sub("℃\[.*\]","",t.text.strip())
